...
app.post('/rfid_history', (req, res) => {
  ...
  pool.getConnection((err, connection) => {
    ...
    // Perform the database query
    connection.query('INSERT INTO tap_history (tap_historyFirstname, tap_historyMiddlename, tap_historyLastname, tap_historyTuptId, tap_historyCourse, tap_historySection, tap_historyEmail, tap_historyDate, user_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)', [firstName, middleName, lastName, tuptId, course, section, email, date, user_id], (error, results) => {
      // Release the connection
      connection.release();
